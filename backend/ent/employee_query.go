...
func (eq *EmployeeQuery) QueryMedicalfiles() *MedicalfileQuery {
	query := &MedicalfileQuery{config: eq.config}
...
func (eq *EmployeeQuery) WithMedicalfiles(opts ...func(*MedicalfileQuery)) *EmployeeQuery {
	query := &MedicalfileQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
	eq.withMedicalfiles = query
	return eq
}
...
func (eq *EmployeeQuery) sqlAll(ctx context.Context) ([]*Employee, error) {
	...
	if query := eq.withMedicalfiles; query != nil {
		...
		query.Where(predicate.Medicalfile(func(s *sql.Selector) {
			s.Where(sql.InValues(employee.MedicalfilesColumn, fks...))
		}))
